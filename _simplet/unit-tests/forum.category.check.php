...
if ( !$Category_Create ) {
	array_push($Return['Errors'], 'Invalid Query (Category_Create): '.mysqli_error($Database['Connection']));
}
...
if (
	...
	$Return['Status'] = 'Failure';
	if ( !$Check_Open ) {
		array_push($Return['Errors'], 'Check_Open returned false.');
	}
	if ( !$Check_Public ) {
		array_push($Return['Errors'], 'Check_Public returned false.');
	}
	if ( !$Check_Private ) {
		array_push($Return['Errors'], 'Check_Private returned false.');
	}
	if ( $Check_None ) {
		array_push($Return['Errors'], 'Check_None returned true.');
	}
	if ( $Check_Status_Open ) {
		array_push($Return['Errors'], 'Check_Status_Open returned true.');
	}
	if ( !$Check_Status_Public ) {
		array_push($Return['Errors'], 'Check_Status_Public returned false.');
	}
	if ( $Check_Status_Private ) {
		array_push($Return['Errors'], 'Check_Status_Private returned true.');
	}
	if ( $Check_Status_None ) {
		array_push($Return['Errors'], 'Check_Status_None returned true.');
	}
	if ( $Check_Auth_Open ) {
		array_push($Return['Errors'], 'Check_Auth_Open returned true.');
	}
	if ( !$Check_Auth_Public ) {
		array_push($Return['Errors'], 'Check_Auth_Public returned false.');
	}
	if ( !$Check_Auth_Private ) {
		array_push($Return['Errors'], 'Check_Auth_Private returned false.');
	}
	if ( $Check_Auth_None ) {
		array_push($Return['Errors'], 'Check_Auth_None returned true.');
	}
}
